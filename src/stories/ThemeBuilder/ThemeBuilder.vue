...
import { TOKEN_USAGE, TOKEN_USAGE_COMPONENTS } from '@/utils/tokenUsage'
...
const filter = ref('')
/**
 * Narrows the list to the tokens one component actually names in its source.
 *
 * The map is generated by `scripts/build-token-usage.mjs` rather than written
 * by hand, so it cannot quietly describe a component that has since moved on.
 * Component-to-tokens rather than the reverse on purpose: several tokens are
 * read by twenty components, and that direction answers a question nobody
 * asked while burying the ones that have two or three consumers.
 */
const component = ref('')
const componentOptions = TOKEN_USAGE_COMPONENTS
...
const visibleGroups = computed(() => {
  ...
  const used = component.value
    ? new Set(TOKEN_USAGE[component.value] ?? [])
    : null
  if (!q && !used) return TOKEN_GROUPS
  return TOKEN_GROUPS.map((g) => ({
    ...g,
    tokens: g.tokens.filter(
      (t) => (!q || t.name.includes(q)) && (!used || used.has(t.name))
    )
  })).filter((g) => g.tokens.length)
})

/** How many tokens the chosen component reads — shown next to the picker. */
const componentTokenCount = computed(() =>
  component.value ? (TOKEN_USAGE[component.value] ?? []).length : 0
)
        ...
        >
          <div :class="$style.TokensHead">
            ...
            <select
              ...
              title="Show only the tokens this component reads"
            >
              <option value="">All components</option>
              <option
                v-for="name in componentOptions"
                :key="name"
                :value="name"
              >
                {{ name }}
            ...
            <span
              v-if="component"
              :class="$style.Manual"
              :title="`${component} names ${componentTokenCount} tokens in its own source`"
            >
              {{ componentTokenCount }} tokens
            </span>
            <input
              v-model="filter"
              :class="[$style.Input, $style.Filter]"
